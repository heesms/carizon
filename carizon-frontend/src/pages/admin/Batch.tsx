...
import { batchApi, BatchJob, BatchJobExecution, BatchWorkflow } from '../../api/admin/batch'
...
export default function Batch() {
  ...
  const jobColumns = [
    ...
    { title: '스케줄', dataIndex: 'cron_expression', key: 'cron_expression', width: 150 },
    {
      title: '상태',
      dataIndex: 'is_active',
      key: 'is_active',
      width: 100,
      render: (active: boolean) => <Tag color={active ? 'green' : 'red'}>{active ? '활성' : '비활성'}</Tag>,
    },
    {
      title: '작업',
      key: 'action',
      ...
      render: (_: any, record: BatchJob) => (
        <Space>
          <Button type="primary" size="small" icon={<PlayCircleOutlined />} onClick={() => handleExecuteJob(record.job_id)} loading={executeJobMutation.isPending}>실행</Button>
          <Button size="small" icon={<HistoryOutlined />} onClick={() => setSelectedJob(record.job_id)}>이력</Button>
        </Space>
      ),
    },
  ]
  ...
  const workflowColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: BatchWorkflow) => (
        <Button type="primary" size="small" icon={<PlayCircleOutlined />} onClick={() => executeWorkflowMutation.mutate({ workflowId: record.workflow_id })} loading={executeWorkflowMutation.isPending}>실행</Button>
      ),
    },
  ]
  ...
  const executionColumns = [
    { title: '실행 ID', dataIndex: 'execution_id', key: 'execution_id', width: 100 },
    {
      ...
      render: (status: string) => {
        const color = status === 'SUCCESS' ? 'green' : status === 'RUNNING' ? 'processing' : status === 'FAILED' ? 'red' : 'default'
    ...
    },
    {
      ...
      render: (_: any, record: BatchJobExecution) => (
        <span>{record.processed_items || 0} / {record.success_items || 0} / {record.failed_items || 0}</span>
      ),
    },
    {
      title: '시작 시간',
    ...
    { title: '에러', dataIndex: 'error_message', key: 'error_message', ellipsis: true },
  ]
          ...
          items={[
            {
              ...
              children: (
                <Space direction="vertical" size="large" style={{ width: '100%' }}>
                  <Table columns={jobColumns} dataSource={jobs} rowKey="job_id" loading={jobsLoading} pagination={false} />
                  {selectedJob && (
                    <Card title={`작업 실행 이력: ${selectedJob}`} size="small">
                      <Table columns={executionColumns} dataSource={executions} rowKey="execution_id" loading={executionsLoading} pagination={{ pageSize: 10 }} size="small" />
                    </Card>
                  )}
                </Space>
              ),
            },
            {
              ...
              label: '워크플로우 관리',
              children: (
                <Table columns={workflowColumns} dataSource={workflows} rowKey="workflow_id" loading={workflowsLoading} pagination={false} />
              ),
            },
          ]}
        />
