...
import { crawlApi, CrawlRun } from '../../api/admin/crawl'
...
export default function Crawl() {
  ...
  const columns = [
    { title: '실행 ID', dataIndex: 'run_id', key: 'run_id', width: 200 },
    {
      title: '플랫폼',
      dataIndex: 'source',
      key: 'source',
    ...
    },
    {
      ...
      render: (status: string) => {
        const color =
          status === 'SUCCESS' ? 'green' : status === 'RUNNING' ? 'processing' : status === 'FAIL' ? 'red' : 'default'
        return <Tag color={color}>{status}</Tag>
      },
    },
    { title: '아이템 수', dataIndex: 'total_items', key: 'total_items', width: 100 },
    {
      title: '시작 시간',
    ...
    { title: '메시지', dataIndex: 'message', key: 'message', ellipsis: true },
  ]
          ...
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <Space wrap>
              {PLATFORMS.map(platform => (
                <Button
                  key={platform.value}
                  onClick={() => runPlatformMutation.mutate(platform.value)}
          ...
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            <Space>
              <Button
                ...
                onClick={() => runMergeMutation.mutate(undefined)}
            ...
            <Space wrap>
              {PLATFORMS.map(platform => (
                <Button
                  key={platform.value}
                  onClick={() => runMergePlatformMutation.mutate({ platform: platform.value })}
