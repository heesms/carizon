...
export default function Config() {
  ...
  const updateMatchingBatchMutation = useMutation({
    mutationFn: (configs: Record<string, number>) => configApi.updateLlmMatchingBatch(configs),
    onSuccess: () => {
  ...
  const promptColumns = [
    { title: '타입', dataIndex: 'prompt_type', key: 'prompt_type', width: 150 },
    {
      ...
      key: 'prompt_text',
      ellipsis: true,
      render: (text: string) => <div style={{ maxWidth: 500, whiteSpace: 'pre-wrap' }}>{text}</div>,
    },
    {
      title: '활성화',
      dataIndex: 'is_active',
      ...
      render: (_: any, record: any) => (
        <Button
          ...
          onClick={() => {
            ...
            promptForm.setFieldsValue({ prompt_text: record.prompt_text, is_active: record.is_active })
          }}
        >
          수정
        </Button>
          ...
          items={[
            {
              ...
              label: '검색 모드',
              children: (
                <Form initialValues={searchMode} onFinish={saveSearchModeMutation.mutate} layout="vertical" style={{ maxWidth: 600 }}>
                  <Form.Item name="useMeilisearch" label="Meilisearch 사용" valuePropName="checked">
                    <Switch />
                  </Form.Item>
                  <Form.Item name="fallbackToDb" label="DB 폴백 사용" valuePropName="checked">
                    <Switch />
                  </Form.Item>
                  <Form.Item>
            ...
            },
            {
              ...
              children: (
                <Space direction="vertical" size="large" style={{ width: '100%' }}>
                  <Table columns={promptColumns} dataSource={prompts} rowKey="id" loading={promptsLoading} pagination={false} />
                  <Modal
                    title="프롬프트 수정"
                    open={!!editingPrompt}
                    onCancel={() => { setEditingPrompt(null); promptForm.resetFields() }}
                    footer={null}
                    width={800}
                  >
                    <Form
                      ...
                      layout="vertical"
                      onFinish={(values) => {
                        if (editingPrompt) updatePromptMutation.mutate({ type: editingPrompt.prompt_type, data: values })
                      }}
                    >
                      <Form.Item name="prompt_text" label="프롬프트 내용" rules={[{ required: true }]}>
                        <TextArea rows={10} />
                      </Form.Item>
              ...
              children: (
                <Space direction="vertical" size="large" style={{ width: '100%' }}>
                  <Card size="small">
                    <Space>
                      ...
                      {presets.map((preset: string) => (
                        <Button key={preset} size="small" onClick={() => applyPresetMutation.mutate(preset)} loading={applyPresetMutation.isPending}>
                          {preset}
                        </Button>
                      ))}
                    </Space>
                  ...
                  <Form
                    form={matchingForm}
                    layout="vertical"
                    onFinish={updateMatchingBatchMutation.mutate}
                  ...
                  >
                      ...
                      <Card key={group} title={group} size="small" style={{ marginBottom: 16 }}>
                        {(configs as any[]).map((config: any) => (
                          <Form.Item
                            ...
                            label={<span>{config.config_key.split('.').pop()}<br /><small style={{ color: '#999' }}>{config.description}</small></span>}
                    ...
                    <Form.Item>
                      <Button type="primary" htmlType="submit" icon={<SaveOutlined />}>일괄 저장</Button>
                    </Form.Item>
                  </Form>
                </Space>
              ),
