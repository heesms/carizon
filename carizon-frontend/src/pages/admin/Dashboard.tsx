...
import { dashboardApi, DashboardStats } from '../../api/admin/dashboard'
...
export default function Dashboard() {
  ...
  const stats = (data?.data?.data || {}) as Partial<DashboardStats>
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginTop: 24 }}>
        <Col span={6}>
          <Card>
            <Statistic title="전체 차량 수" value={carMasterCount} valueStyle={{ color: '#3f8600' }} />
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Statistic title="플랫폼 차량 수" value={platformCarCount} valueStyle={{ color: '#1890ff' }} />
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Statistic title="임베딩된 차량" value={embeddingCount} valueStyle={{ color: '#722ed1' }} />
          </Card>
        </Col>
        <Col span={6}>
          <Card>
            <Statistic title="인덱싱된 차량" value={meilisearchCount} valueStyle={{ color: '#fa8c16' }} />
          </Card>
        </Col>
      </Row>
    </div>
