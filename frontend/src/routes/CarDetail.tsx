...
export default function CarDetail(){
  ...
  useEffect(()=>{ if(!id) return; getCarDetail(id).then(setDetail) }, [id])
  useEffect(()=>{
    if (!id) return
    getPriceHistory(id, platformCarId).then(res=> setHistory(res.points||[]))
  }, [id, platformCarId])
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="text-gray-500">
              <tr><th className="text-left p-2">플랫폼</th><th className="text-left p-2">가격</th><th className="text-left p-2">상태</th><th className="text-left p-2">링크</th><th className="text-left p-2">최근관측</th></tr>
            </thead>
            <tbody>
              {platforms.map(p=>(
                <tr key={p.platformCarId} className="border-t">
                  ...
                  <td className="p-2">
                    {p.pcUrl ? <a href={p.pcUrl} target="_blank" className="text-blue-600 underline">원문</a> : '-'}
                  </td>
