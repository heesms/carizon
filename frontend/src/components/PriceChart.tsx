...
type Props = { points: PricePoint[], width?: number, height?: number }
export default function PriceChart({ points, width=600, height=220 }: Props){
  ...
  const d = points.map((p,i)=> `${i?'L':'M'} ${X(new Date(p.ts).getTime()).toFixed(1)} ${Y(p.price).toFixed(1)}`).join(' ')

  const yTicks = [ymin, Math.round((ymin+ymax)/2), ymax]
  const xTicks = [xmin, xmax]

  return (
    <svg width={width} height={height} role="img" aria-label="가격 히스토리" className="bg-white border rounded">
      <g stroke="#e5e7eb">
        {/* axes */}
        ...
        {xTicks.map((t,i)=> <text key={i} x={X(t)} y={height-4} fontSize="10" textAnchor="middle" fill="#6b7280">
          {new Date(t).toLocaleDateString()}
        </text>)}
      </g>
      <path d={d} fill="none" stroke="#111827" strokeWidth="2"/>
      {points.map((p,i)=> <circle key={i} cx={X(new Date(p.ts).getTime())} cy={Y(p.price)} r="3" fill="#111827" />)}
    </svg>
  )
}
