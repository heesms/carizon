...
import { blogPostApi } from '../api/blogPost'
    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card title="WordPress 포스팅 생성">
        ...
        >
          <Form.Item
            name="modelCode"
            label="모델 코드 (MODEL_CODE)"
            rules={[{ required: true, message: '모델 코드를 입력하세요' }]}
            extra="예: KIA_CORANDO_SPORT_2023"
          >
            <Input
              placeholder="모델 코드 입력"
              style={{ width: '100%' }}
              allowClear
            />
          </Form.Item>
        ...
        <Card title={postResult.preview ? '포스팅 내용 미리보기' : '포스팅 결과'}>
          {postResult.preview ? (
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              />
              <Alert
                message={`선정된 매물: ${postResult.carCount}개`}
                type="success"
                showIcon
              />
            </Space>
