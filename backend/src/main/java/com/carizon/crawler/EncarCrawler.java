...
public class EncarCrawler {
    ...
    public int runOnce() {
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (isProxyAuthError(e)) {
                        ...
                        log.warn("[ENCAR] proxy-auth(407) detected, pause {} ms then retry", pauseMs);
            ...
            log.info("[ENCAR] use_yn dedupe done: updatedRows={}", dedupeUpdated);

            if (ENABLE_ENCAR_EXTRA_APIS && ENRICH_EXTRA_AFTER_BASE) {
                try {
                    int enriched = enrichExtrasAfterBase();
                    log.info("[ENCAR] extra enrichment done: updatedRows={}", enriched);
                } catch (Exception enrichEx) {
                    log.warn("[ENCAR] extra enrichment failed but base crawl already completed: {}", enrichEx.toString());
                }
            }
    ...
    /**
     * DB 적재 없이 목록 페이징만 순회하여 건수를 점검한다.
    ...
    public Map<String, Object> runPagingCountOnly(Integer maxPages) {
        ...
        while (true) {
            ...
            try {
                Object any = getJsonAny(url.toString());
                if (!(any instanceof Map)) {
                    log.warn("[ENCAR][PAGING-TEST] list response is not Map, exit");
                    break;
                }
                Map<String, Object> obj = (Map<String, Object>) any;
                if (apiCount == null) {
                ...
                if (list.isEmpty()) {
                    log.info("[ENCAR][PAGING-TEST] SearchResults empty, exit");
                    break;
                }

                pageNo++;
                totalFetched += list.size();
                log.info("[ENCAR][PAGING-TEST] page={} batch={} total={} nextCursor={}",
                        pageNo, list.size(), totalFetched, nextCursor.isBlank() ? "-" : nextCursor);

                if (maxPages != null && maxPages > 0 && pageNo >= maxPages) {
                    log.info("[ENCAR][PAGING-TEST] maxPages reached: {}", maxPages);
                    break;
                }

                if (list.size() < PAGE_SIZE) {
                    log.info("[ENCAR][PAGING-TEST] last page detected, exit");
                    break;
                }
                ...
                log.warn("[ENCAR][PAGING-TEST] list error: {}", e.toString());
                if (++tryCount > 5) {
                    log.warn("[ENCAR][PAGING-TEST] retry limit exceeded, exit");
                    break;
                }
                try {
                    Thread.sleep(1000L * tryCount);
        ...
        log.info("[ENCAR][PAGING-TEST] done pages={} fetchedByPaging={} apiCount={} elapsedSec={}",
                pageNo, totalFetched, apiCount, elapsedSec);

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("source", "ENCAR");
    ...
    /** 크롤링 없이 raw_encar의 use_yn만 재계산 */
    public Map<String, Object> refreshUseYnOnly() {
        int updatedRows = markUseYnByVehicleNo();
        Integer yCount = jdbc.queryForObject("SELECT COUNT(*) FROM raw_encar WHERE use_yn = 'Y'", Integer.class);
        Integer nCount = jdbc.queryForObject("SELECT COUNT(*) FROM raw_encar WHERE use_yn = 'N'", Integer.class);
        Integer totalCount = jdbc.queryForObject("SELECT COUNT(*) FROM raw_encar", Integer.class);
    ...
    private int handleDetails(List<Map<String, Object>> list) throws Exception {
        ...
        log.info("[ENCAR] detail fetched vehicles={} requestedIds={}", allVehicles.size(), requestedIds);
        ...
        if (enrichInline) {
            ...
            log.info("[ENCAR] extra api target(normal only)={} calledNormal={} filteredNotNormal={} filteredUnknown={}",
                    extraFutureByVehicleId.size(), calledNormal, filteredNotNormal, filteredUnknown);
        }

        String sql = "INSERT INTO raw_encar(payload, car_image_url, price_new, option_array, sel_option_array, seat_count, " +
                    ...
                    if (vehicleId != null && !vehicleId.isBlank()) {
                        ...
                        if (extraFuture != null) {
                            try {
                                ...
                                log.debug("[ENCAR] extra collect failed vehicleId={} err={}", vehicleId, e.toString());
                            }
                        }
                    }
                }
        ...
        while (true) {
            List<ExtraEnrichTarget> targets = jdbc.query(selectSql, (rs, rowNum) -> {
                ...
                if (payloadJson != null && !payloadJson.isBlank()) {
                    try {
                        ...
                        log.debug("[ENCAR] payload parse failed id={} err={}", id, e.toString());
                    }
                }
            ...
            for (CompletableFuture<ExtraEnrichResult> f : futures) {
                try {
                    ...
                    log.debug("[ENCAR] extra enrichment future failed: {}", e.toString());
                }
            }

            if (results.isEmpty()) {
                log.info("[ENCAR] extra enrichment progress: targets={} results=0 updated=0 lastId={}", targets.size(), cursor);
                continue;
            }
            ...
            log.info("[ENCAR] extra enrichment progress: targets={} results={} updated={} lastId={}",
                    targets.size(), results.size(), updatedRows, cursor);
        }

        log.info("[ENCAR] extra enrichment summary: targets={} updated={}", totalTargets, totalUpdated);
        return totalUpdated;
    }
    ...
    private int markUseYnByVehicleNo() {
        jdbc.update("UPDATE raw_encar SET use_yn = 'Y'");
        return jdbc.update("""
                UPDATE raw_encar r
                JOIN (
                    SELECT id,
    ...
    private EncarExtraData fetchEncarExtraData(String vehicleId, String vehicleNo, Map<String, Object> vehicleSummary) {
        try {
            ...
            Map<String, Object> detail = needsOptionSpecFetch
                    ...
                    : baseDetail;
            log.debug("[ENCAR][OPTION] vehicleId={} needsOptionSpecFetch={} hasOptions={} hasSeatCount={}",
            ...
            if ((selOptionArray == null || selOptionArray.isBlank())
                ...
                log.debug("[ENCAR][OPTION] vehicleId={} sel_option_array fallback from choice api names count={}",
                        vehicleId, choiceOptionMap.size());
            }
            Integer seatCount = extractSeatCount(detail);
            log.debug("[ENCAR][OPTION] vehicleId={} standardCodes={} choiceCodes={} optionMapped={} selOptionMapped={} seatCount={}",
                    vehicleId,
            ...
            log.debug("[ENCAR] extra fetch failed vehicleId={} err={}", vehicleId, e.toString());
    ...
    private Map<String, Object> fetchVehicleDetailForOptionAndSpec(String vehicleId) {
        ...
        try {
            ...
            log.debug("[ENCAR] option/spec detail fetch failed vehicleId={} err={}", vehicleId, e.toString());
        }
        return Map.of();
    }
    ...
    private Map<String, Object> fetchVehicleOpenRecord(String vehicleId, String vehicleNo) {
        ...
        try {
            ...
            if (!isNotFoundError(e)) {
                log.debug("[ENCAR] open record fetch failed vehicleId={} err={}", vehicleId, e.toString());
            }
        }
        return Map.of();
    }
    ...
    private Map<String, String> fetchOptionCodeNameMapByUrls(List<String> urls, String optionType, String vehicleId) {
        for (String url : urls) {
            try {
                log.debug("[ENCAR][OPTION] request type={} vehicleId={} url={}", optionType, vehicleId, url);
                ...
                if (!codeNameMap.isEmpty()) {
                    log.debug("[ENCAR][OPTION] response type={} vehicleId={} mappedCodes={}", optionType, vehicleId, codeNameMap.size());
                    return codeNameMap;
                }
                log.debug("[ENCAR][OPTION] empty map type={} vehicleId={} url={}", optionType, vehicleId, url);
            } catch (Exception e) {
                if (isNotFoundError(e)) {
                    log.debug("[ENCAR][OPTION] 404 type={} vehicleId={} url={}", optionType, vehicleId, url);
                    continue;
                }
                log.debug("[ENCAR] {} option map fetch failed vehicleId={} url={} err={}", optionType, vehicleId, url, e.toString());
            }
        }
        return Map.of();
    }
